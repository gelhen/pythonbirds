...
class Fase():
    ...
    def status(self):
        ...
        :return:
        """
        if not self._checa_ator_ativo(self._porcos):
            return VITORIA
        if self._checa_ator_ativo(self._porcos) and self._checa_ator_ativo(self._passaros):
            return EM_ANDAMENTO
        return  DERROTA

    def _checa_ator_ativo(self, atores):
        for a in atores:
            if a.status == ATIVO:
                return True
        return False
    ...
    def lancar(self, angulo, tempo):
        ...
        if len(self._passaros) > 0:
            for _passaro in self._passaros:
                if not _passaro.foi_lancado():
                    _passaro.lancar(angulo, tempo)
                    break
    ...
    def calcular_pontos(self, tempo):
        ...
        for passaro in self._passaros:
            ...
            for obstaculo_ou_porco in chain(self._obstaculos, self._porcos):
                passaro.colidir(obstaculo_ou_porco, self.intervalo_de_colisao)

            passaro.colidir_com_chao()

        return [self._transformar_em_ponto(a) for a in
                self._passaros + self._obstaculos + self._porcos]
